...
const sectionFields: [string, string, boolean][] = [
  ["logos", "Partner / Brand Logos", false], ["whoWeWorkWith", "Who We Work With", true],
  ["journey", "Trade Journey / Process", true], ["stats", "Stats", false],
  ["projects", "Featured / Commercial Projects", true], ["benefits", "Why Partner With T1", true],
  ["partnershipServices", "Partnership Services", true], ["industryServices", "Industry / Trade Services", true],
  ["resources", "Resource Center", true], ["supplierCTA", "Preferred Supplier CTA", true],
  ["designTips", "Design Tips & Insights", true], ["referral", "Referral Partnership", true],
  ["consultation", "Consultation / Lead Form", true], ["faq", "FAQ", true],
]

export function TradePageFormClient({ initialData }: { initialData: TradePageApiData }) {
  ...
  const submit = handleSubmit(async (data) => {
    setSaving(true)
    try {
      const response = await fetch("/api/save-trade-page", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) })
  ...
  return <form onSubmit={submit} className="space-y-4 pb-24">
    ...
    <SectionAccordion title="Hero" order={1} control={control} visibleName="sections.hero.isVisible" defaultOpen>
      <LocalizedField control={control} name="sections.hero.eyebrow" label="Eyebrow" /><LocalizedField control={control} name="sections.hero.heading" label="Heading" multiline /><LocalizedField control={control} name="sections.hero.description" label="Description" multiline /><ImageField control={control} name="sections.hero.backgroundImage" label="Background Image" /><ImageField control={control} name="sections.hero.mobileImage" label="Mobile Image" /><ButtonField control={control} name="sections.hero.primaryButton" label="Primary Button" /><ButtonField control={control} name="sections.hero.secondaryButton" label="Secondary Button" /><PlainField control={control} name="sections.hero.overlayOpacity" label="Overlay Opacity" type="number" />
    </SectionAccordion>
    {sectionFields.map(([name, title, hasEyebrow], index) => <TradeSectionForm key={name} control={control} name={name} title={title} order={index + 2} hasEyebrow={hasEyebrow} />)}
...
function TradeSectionForm({ control, name, title, order, hasEyebrow }: { control: Control<TradePageApiData>; name: string; title: string; order: number; hasEyebrow: boolean }) {
