...
import type { CmsSection, FeaturedSpacesContent, Language } from "@/types/cms"

type FeaturedSpacesTab = "content" | "media" | "settings"

const TABS: { key: FeaturedSpacesTab; label: string }[] = [
...
export function FeaturedSpacesSectionDetail({
  ...
  const [activeTab, setActiveTab] = useState<FeaturedSpacesTab>("content")
  ...
  return (
    ...
    >
      ...
      >
        {TABS.map(({ key, label }) => (
          <button
            key={key}
            role="tab"
            id={`fs-tab-${section.id}-${key}`}
            aria-selected={activeTab === key}
            aria-controls={`fs-panel-${section.id}-${key}`}
      ...
      <div
        role="tabpanel"
        id={`fs-panel-${section.id}-content`}
        aria-labelledby={`fs-tab-${section.id}-content`}
      ...
      <div
        role="tabpanel"
        id={`fs-panel-${section.id}-media`}
        aria-labelledby={`fs-tab-${section.id}-media`}
      ...
      <div
        role="tabpanel"
        id={`fs-panel-${section.id}-settings`}
        aria-labelledby={`fs-tab-${section.id}-settings`}
