...
import type { CmsSection, Language, StatsContent } from "@/types/cms"

type StatsTab = "content" | "media" | "settings"
...
export function StatsSectionDetail({
  ...
  const [activeTab, setActiveTab] = useState<StatsTab>("content")
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 border-b border-zinc-100 px-4 py-3">
        ...
        {/* EN / AR language switcher — in header for stats */}
        <div
          role="group"
      ...
      >
        {TABS.map(({ key, label }) => (
          <button
            key={key}
            role="tab"
            id={`stats-tab-${section.id}-${key}`}
            aria-selected={activeTab === key}
            aria-controls={`stats-panel-${section.id}-${key}`}
      ...
      <div
        role="tabpanel"
        id={`stats-panel-${section.id}-content`}
        aria-labelledby={`stats-tab-${section.id}-content`}
      ...
      <div
        role="tabpanel"
        id={`stats-panel-${section.id}-media`}
        aria-labelledby={`stats-tab-${section.id}-media`}
      ...
      <div
        role="tabpanel"
        id={`stats-panel-${section.id}-settings`}
        aria-labelledby={`stats-tab-${section.id}-settings`}
        hidden={activeTab !== "settings"}
      >
        {activeTab === "settings" && (
          <p className="px-4 py-6 text-sm text-zinc-500">
            Layout and visibility settings coming soon.
          </p>
        )}
      </div>
