...
import type { LocationLinksFormValues } from "./location-links-content-tab"
import type { Language } from "@/types/cms"

interface LocationColumnCardProps {
  colIndex: number
  language: Language
  control: Control<LocationLinksFormValues>
  register: UseFormRegister<LocationLinksFormValues>
  onRemoveColumn: () => void
}

export function LocationColumnCard({
  colIndex,
  language,
  control,
  register,
  onRemoveColumn,
}: LocationColumnCardProps) {
    ...
    <div className="rounded-lg border border-zinc-200 bg-white">
      <div className="flex items-center gap-3 px-4 pb-3 pt-4">
        ...
        <div className="min-w-0 flex-1">
          <label className="mb-1 block text-xs font-medium text-zinc-500">Column Title</label>
          <Input
            {...register(`columns.${colIndex}.title${l}` as `columns.${number}.titleEn`)}
            dir={dir}
            lang={lang}
            placeholder={isEn ? "Location name" : "اسم الموقع"}
      ...
      <div className="border-t border-zinc-100 px-4 pb-4 pt-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          {linkFields.map((link, linkIndex) => (
            ...
            >
              {/* Mini grip — too small for DragHandle abstraction */}
              ...
              {/* Mini delete — too small for DeleteButton abstraction */}
              <button
                type="button"
        ...
        <button
          type="button"
          onClick={() => appendLink({ id: `ll-${Date.now()}`, label: "", href: "" })}
